...
def generate_pages():
    ...
    for lang_code, data_for_lang in all_content.items():
        ...
        for template_filename in template_files:
            ...
            # Ensure all expected global contexts exist
            for key in ['assets','navigation','colors','layout','security','footer','books','content']:
                context.setdefault(key, {})

            # Provide solar_data for JS usage
            if 'solar_data' in data_for_lang:
                context['solar_data'] = data_for_lang['solar_data']
            else:
                # Fallback to books data if exact solar_data not provided
                context['solar_data'] = {'books': context.get('books', {})}
