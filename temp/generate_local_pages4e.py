...
def generate_pages():
    ...
    for lang_code, data_for_lang in all_content.items():
        ...
        for template_filename in template_files:
            ...
            # Page-specific data: try direct or '_page' suffix
            ...
            context = {
                ...
                page_key: page_data,
                'page': page_data,
                'content': page_data  # for templates using content.*
            }
            # Inject all global sections from JSON (fallback to empty dict)
            for global_key in ['assets', 'navigation', 'colors', 'layout', 'security', 'footer']:
                context[global_key] = data_for_lang.get(global_key, {})

            # Solar data for JS seeds
            if 'solar_data' in data_for_lang:
                context['solar_data'] = data_for_lang['solar_data']
            else:
                context['solar_data'] = {'books': data_for_lang.get('books', {})}

            # Provide direct 'books' for templates
            context['books'] = context['solar_data'].get('books', {})
